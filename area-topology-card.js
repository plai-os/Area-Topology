const CARD_VERSION = "1.27.6";
const BUILD_COMMIT = "60b02bf";
...
class AreaTopologyCard extends HTMLElement {
  ...
  lcarsRailValue(config, view = {}) {
    ...
    const entityId = config.entity || (String(config.source || "").toLowerCase() === "weather" ? view.entity : "");
  ...
  renderLcarsFromViews() {
    ...
    const kioskUrl = String(this._config.kiosk_url || `${window.location.origin}${window.location.pathname}?kiosk`);
    const qrColor = selectedColor.replace("#", "");
    const kioskQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=8&format=svg&color=050507&bgcolor=${encodeURIComponent(qrColor)}&data=${encodeURIComponent(kioskUrl)}`;
    ...
    return `<div class="lcars-dashboard lcars-kiosk ${urlKiosk ? "lcars-url-kiosk" : ""} bridge-active" style="--lcars-header:${headerColor};--lcars-header-contrast:${contrastColor(headerColor)};--lcars-datetime:${dateTimeColor};--lcars-kiosk-height:${kioskHeight}">
      ...
      }).join("")}<div class="lcars-nav-foot"></div><a class="lcars-kiosk-qr" href="${escapeHtml(kioskUrl)}" target="_blank" rel="noopener" style="--qr-tone:${selectedColor};--qr-contrast:${contrastColor(selectedColor)}" title="Open this dashboard in kiosk mode"><span>KIOSK ACCESS</span><img src="${escapeHtml(kioskQrUrl)}" alt="QR code to open ${escapeHtml(this._config.title)} in kiosk mode"><b>SCAN TO CONNECT</b></a></nav><main class="lcars-main">${content}<div class="lcars-footer" style="--lcars-footer-tone:${selectedColor}"><span></span><b>VERSION ${CARD_VERSION} // BUILD ${BUILD_COMMIT}</b><i></i></div></main></div>
    </div>`;
  }
  ...
  styles() { return `
    ...
    * { box-sizing:border-box; }
    ...
    .lcars-kiosk .lcars-main { min-height:0; display:grid; grid-template-rows:minmax(0,1fr) auto; overflow:hidden; }
    ...
    .lcars-kiosk .lcars-bridge-shell>aside { min-height:0; height:100%; overflow:hidden; }
    ...
    .lcars-weather-current dl { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }.lcars-weather-current dl>div { min-width:0; margin-top:0; }.lcars-weather-current dt,.lcars-weather-current dd { font-size:18px; }.lcars-weather-current dt ha-icon { --mdc-icon-size:20px; }.lcars-weather-current dd { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
