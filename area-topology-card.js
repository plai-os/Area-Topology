const CARD_VERSION = "1.27.57";
const BUILD_COMMIT = "603acfb";
...
class AreaTopologyCard extends HTMLElement {
  ...
  async configureLcarsHistoryCard() {
    ...
    try {
      ...
      host.append(chart);
      this.styleLcarsPopupContent(chart);
    } catch (error) {
  ...
  styleLcarsPopupContent(card) {
    const css = `
      ...
      :host,ha-card,.card-content,* { --primary-font-family:"LCARS GTJ3","Helvetica LT Std Ultra Compressed","Arial Narrow",sans-serif; --paper-font-common-base_-_font-family:var(--primary-font-family); font-family:var(--primary-font-family)!important; font-weight:400!important; }
      svg text,.chart text,text { font-family:"LCARS GTJ3","Helvetica LT Std Ultra Compressed","Arial Narrow",sans-serif!important; font-weight:400!important; letter-spacing:.025em; }
      h1,h2,h3,.name,.state,.value,.header { font-family:"LCARS GTJ3","Helvetica LT Std Ultra Compressed","Arial Narrow",sans-serif!important; font-weight:400!important; text-transform:uppercase; }
    `;
    const apply = (root) => {
      if (!root) return;
      if (!root.querySelector?.("style[data-lcars-popup-font]")) {
        const style = document.createElement("style");
        style.dataset.lcarsPopupFont = "";
        style.textContent = css;
        root.append(style);
      }
      for (const element of root.querySelectorAll?.("*") || []) if (element.shadowRoot) apply(element.shadowRoot);
      if (!root.__lcarsPopupFontObserver) {
        root.__lcarsPopupFontObserver = new MutationObserver(() => {
          for (const element of root.querySelectorAll?.("*") || []) if (element.shadowRoot) apply(element.shadowRoot);
        });
        root.__lcarsPopupFontObserver.observe(root, { childList:true, subtree:true });
      }
    };
    apply(card.shadowRoot || card);
  }
  ...
  lcarsOverallConnectivity() {
    const devices = [...new Map((this._data || []).flatMap((area) => area.devices || []).map((device) => [device.id, device])).values()];
    const states = devices.map((device) => this.lcarsDeviceConnectivity(device));
    return {
  ...
  styles() { return `
    ...
    * { box-sizing:border-box; }
    ...
    .lcars-status-ribbon { position:absolute; z-index:8; bottom:-23px; left:85%; display:grid; grid-template-columns:minmax(145px,1fr) minmax(155px,1fr) minmax(145px,1fr); width:min(520px,42vw); height:46px; padding:5px; border-radius:25px; background:#050507; transform:translateX(-50%); pointer-events:none; }
    ...
    .lcars-status-ribbon .online { margin-left:5px; background:color-mix(in srgb,var(--bridge-tone) 34%,#050507); }
    ...
    .lcars-bridge-grid { z-index:3; padding-top:calc(var(--bridge-gap) + 18px); padding-bottom:var(--bridge-gap); }
    ...
    .lcars-area>header>button[data-area-config] { padding-top:10px; padding-bottom:4px; transform:translateY(5px); }
    .lcars-area-reading { padding-top:8px; padding-bottom:2px; transform:translateY(5px); }
