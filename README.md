...
The complete reorganised example is in [`examples/lcars-home-card-v2.yaml`](examples/lcars-home-card-v2.yaml). Existing `floors`, `weather`, `security`, `engineering`, `captains_log`, `bridge`, and `menus` configurations remain supported when `views:` is omitted. Device sections can use `exclude_labels` and `exclude_devices` to keep broad queries in separate cards—for example, selecting `Climate` while excluding `Appliances` from Environmental Systems.
...
bridge:
  ...
  # motion_sensor: binary_sensor.car_camera_motion
  # recording_sensor: binary_sensor.car_camera_recording
...
Adding `bridge` creates a numbered `BRIDGE` destination using live devices from the configured floor. `primary_area` fills the left command module, remaining areas stack on the right, and `camera` reuses the matching Security feed with an LCARS surveillance frame and optional telemetry sensors.
